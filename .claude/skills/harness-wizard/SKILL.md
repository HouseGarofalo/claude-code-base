...
# Harness Setup Wizard Skill

Interactive wizard for setting up autonomous long-running agent harnesses. Creates fully-configured autonomous coding projects that can work across multiple sessions with proper state management and agent handoffs.

## Triggers

Use this skill when:
- Setting up a new autonomous harness project
- Creating multi-session development environment
- Configuring autonomous coding workflow
- Resuming an existing harness project
- Keywords: harness wizard, setup harness, create harness, harness configuration, autonomous setup, multi-session project
...
Use harness-initializer skill

# Start coding session
Use harness-coder skill

# Check status
Query Archon for project tasks

# Resume project
Use harness-coder skill with resume mode
```
```
...
echo ""
echo "Next steps:"
echo "  1. Use harness-initializer skill to start initialization"
...
### Option 1: Initialize Now
Use the harness-initializer skill to start setup.

### Option 2: Manual Start
Run `./init.sh` then use harness-initializer skill.
...
```
Warning: Archon MCP server is not accessible.
...
```
Warning: Harness configuration already exists in this directory.

Options:
1. Resume existing project (use harness-coder skill)
2. Create new project in subdirectory
3. Overwrite existing configuration (destructive)
4. Cancel setup
```
...
When setting up, follow these guidelines:
...
6. **Trust the pipeline**: Let Testing and Review skills do their jobs
